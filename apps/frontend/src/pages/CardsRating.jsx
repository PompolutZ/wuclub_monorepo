    ...
    firebase
      ...
      .then((snap) => {
        ...
        const cardRankingsPerFaction = Object.entries(
          ...
          const factionSummary = data.reduce((decksAcc, deck) => {
            const ratings = deck.cards.reduce((acc, card) => {
              return {
                ...acc,
                [card]:
                  ((deck.lastModified - baseDate) / (new Date() - baseDate)) *
          ...
          const normalizedRatings = Object.entries(factionSummary).reduce(
            (acc, [card, rating]) => {
              return {
                ...acc,
                [card]: Math.round(Math.round((rating / maxRating) * 100) / 10),
              };
            },
            {},
          );
        ...
        const normalizedGenericRanks = Object.entries(genericRatings).reduce(
          (acc, [card, rating]) => {
            return {
              ...acc,
              [card]: Math.round(
