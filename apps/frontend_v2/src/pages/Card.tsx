...
import { getSetById, validateCardForPlayFormat } from "../data/wudb";
  ...
  return (
    <div className="w-full h-full lg:w-1/2 lg:mx-auto grid place-content-center px-4 text-gray-900">
      ...
      {card && (
        <>
          <div>
            <h3 className="font-bold text-gray-700 mt-4">Card location:</h3>
            <div>
              <ExpansionPicture
                setName={sets[card.setId].name}
          ...
          <div>
            ...
            <div className="flex my-2 items-center space-x-4">
              {isValid && (
                <span>✅ This card is VALID for Nemesis format.</span>
              )}
              {isRestrictedNemesis && (
                <span>⚠️ This card is RESTRICTED for Nemesis format.</span>
              )}
              {isForsakenNemesis && (
                <span>❌ This card is FORSAKEN for Nemesis format.</span>
              )}
            </div>
          </div>
        </>
