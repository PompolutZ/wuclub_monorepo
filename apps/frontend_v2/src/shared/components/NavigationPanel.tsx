...
export default function NavigationPanel() {
  ...
  const isHomePage = new RegExp(/^\/$/).test(pathname);

  return (
    <header
      className={`p-3 sm:px-8 flex items-center relative ${isHomePage ? "" : "bg-zinc-300"}`}
    >
      <div className="mr-8">
        <Link
      ...
      <MobileMenu
        className={`text-2xl stroke-current stroke-2 ml-auto lg:hidden ${
          new RegExp(/^\/$/).test(pathname) ? "text-white" : "text-gray-900"
        }`}
      />
    </header>
  );
